 ...
 * 
 * Lot of help was made from the following links: 
 * https://zhuanlan.zhihu.com/p/405184256
 * https://github.com/myisabella/tshlab/blob/master/tsh.c
 * 
 */
#include <assert.h>
#include <stdio.h>
...
//Handles Quit command from shell
//For signal blocking issues, while trial
//to implement all commands here was made, rest of the commands are dealt in eval
void builtinCommands(struct cmdline_tokens* command){
    if (command->builtins == BUILTIN_QUIT){
        exit(0);
    }
}

//Funvtion to redirect file descriptor
//If infile != null, redirect stdin to inVal
//If outfile != null, rediret stdout to outVal
void redirectIO(struct cmdline_tokens* command){
    int outVal,inVal;
    if(command->outfile == NULL)
        outVal = 1;
    else if ((outVal = open(command->outfile, O_RDWR|O_CREAT)) < 0)
        printf("failed to open or create file %s\n",command->outfile);
    if(command->infile == NULL)
        inVal = 0;
    else if ((inVal = open(command->infile, O_RDONLY)) < 0)
        printf("failed to open or create file %s\n",command->infile);
    dup2(inVal,0);
    dup2(outVal,1);
...
void eval(char *cmdline) 
{
    ...
    pid_t pid = 0;
    sigset_t allMask, maskOne, maskPrev, maskTwo;
   
    sigemptyset(&maskOne);
    sigaddset(&maskOne, SIGCHLD);
    sigfillset(&allMask);
    sigemptyset(&maskTwo);
    sigaddset(&maskTwo, SIGINT);
    sigaddset(&maskTwo, SIGCHLD);
    sigaddset(&maskTwo, SIGTSTP);
    ...
    if (tok.builtins == BUILTIN_BG){    
        ...
        sigprocmask(SIG_BLOCK, &allMask, &maskPrev);        
        switch(job->state){
            case ST:
                job->state = BG;
    ...
    if (tok.builtins == BUILTIN_NOHUP){
        ...
        sigprocmask(SIG_BLOCK, &mask_hup, &maskPrev);
        sigprocmask(SIG_BLOCK, &maskOne, &mask_tmp);
        pid = fork();
        if (pid == 0)
        {
    ...
    sigprocmask(SIG_BLOCK, &maskTwo, &maskPrev);
    ...
    if ((pid = fork()) == 0){
        ...
        if (bg == 0)
            ...
            addjob(job_list, pid, 2, cmdline);
        sigprocmask(SIG_SETMASK, &maskTwo, NULL);
        if (bg == 0)
        {
            while(pid == fgpid(job_list)){
                sigsuspend(&maskPrev);
...
sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &stat, WNOHANG | WUNTRACED)) > 0) {
        ...
        if(WIFSTOPPED(stat)){
            struct job_t* tempjob = getjobpid(job_list, pid);
