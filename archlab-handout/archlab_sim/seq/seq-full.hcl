##Archlab seq-full.hcl, written by 张晙优 Percy Joonwoo Jang, 1800094804@pku.edu.cn

#Instruction required for IADDQ instruction is as following:
#Fetch: 
#icode:ifun <- M1[PC]
#rA: rB <- M1[PC+1]
#valC <- M8[PC+2]
...
#Instruction required for JM instruction is as following (jm rB, V):
#Fetch: 
#icode:ifun <- M1[PC]
#rA: rB <- M1[PC+1]
#valC <- M8[PC+2]
...
#PC <- valM
#
#Corresponding instructions were added to implement the above instructions in SEQ Hardware.
...
word new_pc = [
	...
	icode == IRET : valM;
	# For JM, new PC will be to the address at M[rB+V] = valM
	icode == IJM : valM;
